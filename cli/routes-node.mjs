...
async function resolveHarnessModule() {
  if (harnessModule) return harnessModule

  const loaderPath = path.join(moduleDir(), '..', 'dist', 'harness', 'loader.js')
  harnessModule = await import(pathToFileURL(loaderPath).href)

  await harnessModule.loadPlugins()
